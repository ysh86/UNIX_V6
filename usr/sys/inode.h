...
struct	inode
{
	...
	short	i_dev;		/* device where inode resides */
	short	i_number;	/* i number, 1-to-1 with device address */
	short	i_mode;
	char	i_nlink;	/* directory entries */
	char	i_uid;		/* owner */
	char	i_gid;		/* group of owner */
	char	i_size0;	/* most significant of size */
	unsigned short	i_size1;	/* least sig */
	short	i_addr[8];	/* device addresses constituting file */
	short	i_lastr;	/* last logical block read (for read-ahead) */
};
